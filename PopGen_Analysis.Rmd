...
Host <- outlierdf$Host
#Make a dataframe
pca_df <- data.frame(sample.id = outlierdf$Sample_ID,
    Geographic_Pop = outlierdf$Geographic_Pop,
    EV1 = pca$eigenvect[,1],    # the first eigenvector
    EV2 = pca$eigenvect[,2],    # the second eigenvector
    stringsAsFactors = FALSE)

pca_df$Geographic_Pop <- as.factor(pca_df$Geographic_Pop)

pca_df <- pca_df %>%
  mutate(Geographic_Pop = recode(Geographic_Pop,
                                  `1` = "SoCal",
                                  `2` = "Central",
                                  `3` = "Marin",
                                  `4` = "Mendocino",
...
custom_colors <- c("darkgoldenrod2", "#377EB8", "#4DAF4A", "#984EA3", "#E41A1C")
unique_pops <- unique(pca_df$Geographic_Pop)
pop_colors <- setNames(custom_colors[1:length(unique_pops)], unique_pops)

LD_pruned_pca <- ggplot(pca_df, aes(x=EV1, y = EV2, color = Geographic_Pop)) + 
  geom_point(size = 3.5, aes(shape = Host)) + xlab("PC1 (3.14%)") + ylab("PC2 (2.82%)") + 
  scale_colour_manual(values = pop_colors) + 
  theme_bw() + theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank())
LD_pruned_pca


tiff("LD-pruned_PCA.tiff", units="in", width=7, height=4, res=400)
LD_pruned_pca
dev.off()
...
custom_colors <- c("darkgoldenrod2", "#377EB8", "#4DAF4A", "#984EA3", "#E41A1C")
tiff("ca_base_blank.tiff", , units="in", width=5, height=8, res=300)
ca_base + theme(
...
ca_base + 
    geom_point(
        ...
        aes(x = Longitude, y = Latitude, shape = Host, size = Count, color = Population), 
        inherit.aes = FALSE
    ) + theme(
        panel.grid.major = element_blank(),  # Remove major grid lines
        panel.grid.minor = element_blank()   # Remove minor grid lines
...
tiff("ca_base.tiff", , units="in", width=5, height=8, res=300)
 ca_base + 
    geom_point(
        data = SampleCoordinates, 
        aes(x = Longitude, y = Latitude, shape = Host, size = Count, color = Population), 
